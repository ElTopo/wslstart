...
When it starts, wslstart tries to run /etc/wslstart.sh script (in which user can start tasks they want) and optinally ~user/.wslstart.sh script (define $WSLUSER in /etc/default/wslstart).
